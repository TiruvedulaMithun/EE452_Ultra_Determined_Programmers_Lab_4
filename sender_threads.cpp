...
void * receiveACK(void * arg)
{
    ...
    {
        printf("here");
        n=listen(send_sock, data, 256, 0, (struct sockaddr *)&from, &length);
...
./custom_ftp <receiving_hostname> <receiving_port> <local_filepath_to_transfer>\n");
    ...
    for (int i = 0; i  < NUM_THREADS; ++i) {
    	j = 0;
        j = pthread_create(&recv_threads[i], NULL, &receiveACK, (void*) i);
        if (j) {
    ...
    for (int i = 0; i  < NUM_THREADS; ++i) {
    	j = 0;
        j = pthread_create(&send_threads[i], NULL, &sendPacket, (void*) i);
        if (j) {
            printf("A request can't be procceses.\n");
        }
        printf("%d\n", i);
