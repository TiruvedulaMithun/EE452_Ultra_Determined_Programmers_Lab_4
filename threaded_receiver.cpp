...
Packet readPacket(char * data) {
    Packet packet;
    bool num_found, pay_found;
    ...
    packet.packet_num = stoi(str_data.substr(begin+1, end-begin-1));
...
void *udpListenThread(void *arg) {
    ...
    while (1) {
        ...
        if (recv_len > 0) {
            ...
            // TODO verify checksum (AB)
            bool received_correct = verifyChecksum(buffer);
            if(received_correct) {
                Packet packet = readPacket(buffer);
            }
            
            // TODO send ACK or NACK (BP)
...
void *fileWriterThread(void *arg) {
    ...
    while (1) {
        if (packets_written >= PACKET_WINDOW) {
            // TODO check packet number and order packets in queue before writing to file (TM)
